...
Инструмент для OSINT-разведки, который раздаёт задачи нескольким агентам-воркерам 
и собирает результаты в веб-панели. Работает локально, не требует внешних серверов.
...
```bash
pip install fastapi uvicorn pydantic requests dnspython python-whois \
            ipwhois phonenumbers python-dotenv
pip install holehe
...
## Запуск агентов
Открой второй терминал и вставь (введи) команду
cd agent
python agent.py --count N где N необходимое количество агентов 
...
## Как открыть панель
открыть любой браузер и вставить (ввести) URL
http://127.0.0.1:5555
...
1. Создай Webhook в Discord: правой кнопкой по каналу → Edit Channel → Integrations → Webhooks → New Webhook. Скопируй URL.

2.Скопируй файл server/.env.example в server/.env:

cd server
copy .env.example .env

3. Открой .env и вставь свой Webhook URL:
...
2. Агенты не подключаются — проверь, что сервер запущен и открыт http://127.0.0.1:5555.

3.Панель пустая — открой DevTools (F12) → Console, смотри ошибки.

4.holehe не работает — нужен установленный .exe. Проверь:
...
проект разрабатывается в учебных целях с 2026